...
public class MainActivity extends Activity {
	...
	public static String VERSIONV2 = VERSION2 + "2.5.3";
	public static String VERSIONV0 = "v20181208";
	...
	// 更新部分
	// 更新接口地址
	public static String UPDATE_URL = "http://app.bs516.cc";
	public static String API_URL = "/pays/api/notify";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			mVideoServer.start();
			sendmsg("服务启动成功，端口:" + WEBSEERVER_PORT);

		} catch (Exception e) {
			sendmsg("服务启动失败，错误:" + e.getMessage());
		}
	...
	class BillReceived extends BroadcastReceiver {
		...
		public void onReceive(final Context context, Intent intent) {
			try {
				if (intent.getAction().contentEquals(BILLRECEIVED_ACTION)) {
					...
					DBManager dbManager = new DBManager(CustomApplcation
							.getInstance().getApplicationContext());
					String dt = System.currentTimeMillis() + "";
					dbManager.addOrder(new OrderBean(money, mark, type, no, dt,
							"", 0));
					...
					dbManager.addQrCode(new QrCodeBean(money, mark, type,
							payurl, dt));
					...
					sendmsg(msg);
						...
						SAVEALIPAYCOOKIE_ACTION)) {
					String cookie = intent.getStringExtra("alipaycookie");
					PayHelperUtils
							.updateAlipayCookie(MainActivity.this, cookie);
				} else if (intent.getAction().contentEquals(
						LOGINIDRECEIVED_ACTION)) {
					String loginid = intent.getStringExtra("loginid");
					String type = intent.getStringExtra("type");
					if (!TextUtils.isEmpty(loginid)) {
						if (type.equals("wechat")
								&& !loginid.equals(currentWechat)) {
							sendmsg("当前登录微信账号：" + loginid);
							currentWechat = loginid;
							AbSharedUtil.putString(getApplicationContext(),
									type, loginid);
						} else if (type.equals("alipay")
								&& !loginid.equals(currentAlipay)) {
							sendmsg("当前登录支付宝账号：" + loginid);
							currentAlipay = loginid;
							AbSharedUtil.putString(getApplicationContext(),
									type, loginid);
						} else if (type.equals("qq")
								&& !loginid.equals(currentQQ)) {
						...
						TRADENORECEIVED_ACTION)) {
					// 商家服务
					final String tradeno = intent.getStringExtra("tradeno");
					String cookie = intent.getStringExtra("cookie");
					final DBManager dbManager = new DBManager(CustomApplcation
							.getInstance().getApplicationContext());
					if (!dbManager.isExistTradeNo(tradeno)) {
						...
						try {
										...
										public void onSuccess(
											...
											try {
												...
												Elements elements = document
														.getElementsByClass("trade-info-value");
												if (elements.size() >= 5) {
													dbManager.updateTradeNo(
															tradeno, "1");
													String money = document
															...
															.replace("+", "")
															.replace("-", "");
													String mark = elements.get(
															3).ownText();
													String dt = System
															.currentTimeMillis()
															+ "";
													dbManager
															.addOrder(new OrderBean(
																	money,
																	mark,
																	"alipay",
																	tradeno,
												...
												PayHelperUtils
														.sendmsg(
																context,
																"TRADENORECEIVED_ACTION-->>onSuccess异常"
																		+ e.getMessage());
											}
										}
									});
							...
							PayHelperUtils
									.sendmsg(
											context,
											"TRADENORECEIVED_ACTION异常"
													+ e.getMessage());
						}
					} else {
						sendmsg("出现重复流水号，疑似掉单，5秒后启动补单");
				...
				PayHelperUtils.sendmsg(context,
						"BillReceived异常" + e.getMessage());
			}
		}

		public void notifyapi(String type, final String no, String money,
				String mark, String dt) {
			try {
				final String notifyurl = AbSharedUtil.getString(
						getApplicationContext(), "apiurl");
				String signkey = AbSharedUtil.getString(
						getApplicationContext(), "signkey");
				...
				String account = "";
				if (type.equals("alipay")) {
					account = AbSharedUtil.getString(getApplicationContext(),
							"alipay");
				} else if (type.equals("wechat")) {
					account = AbSharedUtil.getString(getApplicationContext(),
							"wechat");
				...
				String sign = MD5.md5(dt
						...
						+ signkey
						+ AbSharedUtil
								.getString(getApplicationContext(), "ids")
						+ VERSIONV0);
				RequestParams params = new RequestParams();
				params.addBodyParameter("type", type);
				params.addBodyParameter("no", no);
				params.addBodyParameter("version", VERSIONV0);
				params.addBodyParameter("ids",
						AbSharedUtil.getString(getApplicationContext(), "ids"));
				...
				sendmsg("服务器针对（"
						...
						+ AbSharedUtil
								.getString(getApplicationContext(), "ids")
						+ VERSIONV0 + "）进行签名,密钥是" + signkey + "。签名结果是：" + sign);
				if (!TextUtils.isEmpty(account)) {
					params.addBodyParameter("account", account);
				}
				params.addBodyParameter("sign", sign);
				httpUtils.send(HttpMethod.POST, notifyurl + API_URL, params,
						new RequestCallBack<String>() {

							@Override
							public void onFailure(HttpException arg0,
									String arg1) {
								sendmsg("发送异步通知(" + notifyurl + ")异常，服务器" + ""
										+ "" + "异常" + arg1);
								update(no, arg1);
							}
							...
							public void onSuccess(ResponseInfo<String> arg0) {
								...
								if (result.contains("success")) {
									sendmsg("发送异步通知(" + notifyurl + ")成功，服务器返回"
											+ result);
								} else {
									sendmsg("发送异步通知(" + notifyurl + ")失败，服务器返回"
											+ result);
								}
								update(no, result);
							}
		...
		private void update(String no, String result) {
			DBManager dbManager = new DBManager(CustomApplcation.getInstance()
	...
	public void checkVersion() {
		...
		String version_sign = MD5.md5(currentVersionName + VERSION2 + VERSIONV0
				+ version_time + VERSIONVKEY);
		requestParams.addBodyParameter("time", version_time);
		requestParams.addBodyParameter("sign", version_sign);
		HttpUtils httpUtils = new HttpUtils();
		httpUtils.send(HttpMethod.POST, UPDATE_URL, requestParams,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> arg0) {
						...
						try {
							...
							if (code == 1) {
								int versionCode = Integer.parseInt(jsonObject
										.getString("version_code"));
								if (versionCode > currentVersionCode) {
									// 有新版本
									String download_url = jsonObject
											.getString("url");
	...
	public void download(String url) {
		HttpUtils httpUtils = new HttpUtils();
		httpUtils.download(url, "/sdcard/download/payhelper.apk", false, true,
				new RequestCallBack<File>() {
					...
					public void onLoading(long total, long current,
							boolean isUploading) {
						sendmsg("下载进度"
								+ (int) ((double) current / (double) total * 100.0)
								+ "%");
