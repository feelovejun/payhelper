		...
		try {
			...
			XposedBridge.hookAllMethods(clazz, "a", new XC_MethodHook() {
				...
				protected void afterHookedMethod(MethodHookParam param)
					...
					if(PayHelperUtils.getVerName(context).equals("6.7.3")){
			...
			XposedHelpers.findAndHookMethod("com.tencent.mm.plugin.collect.ui.CollectCreateQRCodeUI",appClassLoader, "initView",
				...
				protected void afterHookedMethod(MethodHookParam param)
					...
					if(PayHelperUtils.getVerName(context).equals("6.7.3")){
						...
						//获取WalletFormView控件
						Field WalletFormViewField = XposedHelpers.findField(param.thisObject.getClass(), "iKG");
						...
						//获取金额控件
						Field AefField = XposedHelpers.findField(WalletFormViewClass, "wCS");
						...
						//点击确定
						Button click=(Button)XposedHelpers.callMethod(param.thisObject, "findViewById",2131822647);
						click.performClick();
					}						
					if(PayHelperUtils.getVerName(context).equals("6.6.7")){
						Intent intent = ((Activity) param.thisObject).getIntent();
